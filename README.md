...
AZURE_VOICELIVE_ENDPOINT=wss://your-resource-name.openai.azure.com
...
3.  **Start Talking:**
    ...
    -   Speak in **English, Hindi, or Gujarati**.

    -   Try interrupting the agent while it is speaking to test the "Barge-in" feature.
...
```
📂 hybrid-voice-agent
├── server_hybrid.py   # Main FastAPI Backend (WebSocket Logic)
├── index.html         # Frontend (AudioContext & UI)
...
Standard MP3 streaming often causes "Chipmunk" (fast speed) or latency issues in browsers due to sample rate mismatches (44.1kHz vs 48kHz).

We enforce **24,000 Hz Raw PCM** across the entire stack:

-   **Microphone:** Records at 24kHz (Azure Requirement).

-   **ElevenLabs:** Generates `pcm_24000`.

-   Browser: Decodes raw bytes into a 24000 AudioBuffer.

    Result: Zero latency overhead from decoding and perfect pitch.

🐛 Troubleshooting
------------------

-   **"Connection Error":** Check if your `AZURE_VOICELIVE_MODEL` name matches the **Deployment Name** in Azure AI Studio exactly.

-   **"Voice is too fast/slow":** Ensure both `server_hybrid.py` is set to `output_format="pcm_24000"` and `index.html` creates a buffer with `24000`.

-   **"429 Too Many Requests":** The server uses a queue to limit ElevenLabs requests. If this persists, check your ElevenLabs subscription tier concurrency limits.
